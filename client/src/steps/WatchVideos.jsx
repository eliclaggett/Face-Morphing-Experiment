...
export default function WatchVideos({ next }) {
  ...
  function playbackFunc() {
    ...
    if (elapsedTime >= totalTime) {
      // Show rating UI
      document.querySelectorAll(".ratingUI").forEach((el) => {
  ...
  useEffect(() => {
    ...
    if (loadingTimer == 0) {
      ...
      setTimeout(() => {
        document
          .querySelectorAll(".singleVideoContainer")
  ...
  useEffect(() => {
    ...
    if (!token) player.set("requestLiveKitToken", true);
  ...
  function handleNextVideo() {
    ...
    const convo_id = randomizedVideos[videoIdx].slice(
    ...
    player.set("videoWatchTimes", [
      ...videoWatchTimes,
      { idx: videoIdx, ts: new Date().getTime() },
    ]);
  }
  ...
  function setHolding(name, val) {
    let el = null;
    if (name == "leftLike") {
      el = document.querySelectorAll(".like")[0];
    } else if (name == "rightLike") {
      el = document.querySelectorAll(".like")[1];
    } else if (name == "leftDislike") {
      el = document.querySelectorAll(".dislike")[0];
    } else if (name == "rightDislike") {
      el = document.querySelectorAll(".dislike")[1];
    }
    if (val == -1) {
  ...
  return (
    ...
    >
      <Container className="loadingModal">
        <Typography level="body-md" sx={{ fontSize: "1.5em" }}>
          {loadingTxt}
        </Typography>
        <Typography level="title-lg" sx={{ fontSize: "3em" }}>
          {loadingTimer}
        </Typography>
      </Container>
      <div
      ...
      >
        <Typography level="title-lg">
          Pay attention to the following interaction ({videoIdx + 1} /{" "}
          {gameParams.numVideos})
        </Typography>
        <Typography level="body-md">
          Then, please rate how much you like or dislike each individual
        </Typography>
      </div>
        ...
        >
          ...
          >
            <div className="webcam_box" onClick={handleShowHideWebcam}>
              ...
              <VisibilityOff />
              <Typography level="body-sm" sx={{ fontWeight: 100 }}>
                {webcamTxt}
              </Typography>
            </div>
          </Stack>
      ...
      >
        <div style={{ width: "3.5rem", flexShrink: 0 }}></div>
        <Typography level="body-sm" textAlign={"left"} sx={{ pb: 4 }}>
          {msToTime(timeLeft)}
        </Typography>
      </Container>
...
function MyVideoConference() {
  ...
  const remoteTrackRef = trackRefs.find(
    (trackRef) => trackRef.participant.name === "python-publisher"
  );

  return (
    <div className="singleVideoContainer">
      {remoteTrackRef ? (
        <VideoTrack trackRef={remoteTrackRef}></VideoTrack>
      ) : (
